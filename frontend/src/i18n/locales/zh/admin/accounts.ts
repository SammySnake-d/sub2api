export default {
    accounts: {
      ...
      upstreamBilling: {
        ...
        recheckUnsupported: '该上游不提供倍率接口，无需探测；更换上游后可点此重新判定',
        stale: '已过期',
        unsupported: '不适用',
        unsupportedTitle: '已探测：上游不提供倍率接口',
        unsupportedHint: '该上游不是 Sub2API 中转站，没有可查询的倍率接口，本列对此账号不适用。更换上游后系统会自动重新判定。',
        failed: '失败',
        failedTitle: '已探测：未取得上游声明倍率',
        failureCount: '连续失败 {count} 次',
        failureReasonUnknown: '失败原因：{code}',
        failureReason: {
          transport_unavailable: '失败原因：本地探测通道不可用',
          missing_api_key: '失败原因：账号未配置 API Key',
          invalid_base_url: '失败原因：上游地址不合法',
          proxy_unavailable: '失败原因：账号绑定的代理不可用',
          request_build_failed: '失败原因：探测请求构造失败',
          request_failed: '失败原因：请求上游失败（网络或超时）',
          empty_response: '失败原因：上游返回空响应',
          response_read_failed: '失败原因：上游响应读取失败',
          response_too_large: '失败原因：上游响应过大',
          http_error: '失败原因：上游返回错误状态码',
          invalid_response: '失败原因：上游响应不是可识别的倍率数据'
        },
        httpStatus: '上游返回 HTTP {value}',
        lastAttemptAt: '本次探测时间：{value}',
        notProbed: '未探测',
        notProbedHint: '这个账号还没有探测过；点击右侧按钮立即探测。',
      ...
      usageWindow: {
        ...
        mirasimQuota: {
          unprobed: '未探测',
          probe: '探测',
          probeTooltip: '主动查一次上游 /v1/limits：GET 空请求，不消耗额度',
          probeNoSnapshot: '探测返回 200 但没有快照',
          probeNotPerformed: '本次没有向上游探测，只是重新读取了存量读数',
          probeFailed: '上次探测失败',
          probeFailedTooltip: '最近一次 /v1/limits 探测失败，所以这里还没有任何读数',
          staleReading: '读数可能已过期',
          staleReadingTooltip: '最近一次 /v1/limits 探测失败，下面是上一次成功读数，不是当前值',
          suspended: '上游额度已暂停',
          remainingUnknown: '剩余未知',
          remaining: '剩余 {percent}%',
          overUsed: '已用 {percent}%（超额）',
          usedOfBudget: '已用 {used} / {budget}',
          noReadingTooltip: '本次探测没有返回这个窗口',
          resetAt: '重置 {time}',
          resetUnknown: '重置时间未知',
          snapshotAt: '快照 {time}',
          sourceLimits: '来自 /v1/limits 主动探测：有绝对额度',
          sourceHeaders: '来自响应头被动采样：只有使用率，没有绝对额度',
          planExpiresAt: '到期 {date}',
          planAuthoritative: '上游探测读回的权威档位',
          planClaimed: '导入时声明的档位，未经上游核实（已知会虚标）',
          pendingUpgradeTooltip: '邀请数已达标但升级还没结算，当前仍是低一档',
          restrictedAll: '全部模型受限',
          restrictedPartial: '部分模型受限',
          scopeClaude: 'Claude（非 Fable）',
          scopeFable: 'Fable',
          windowExhausted: '{window} 已用尽',
          unmappedWindow: '未映射窗口 {name}',
          unmappedWindowTooltip: '上游返回的窗口名不在 5h / 7d / 7d_claude / 7d_fable 之内，没有按已知窗口采纳'
        }
      },
      openaiQuotaReset: {
        count: '次数',
