export default {
    accounts: {
      ...
      upstreamBilling: {
        ...
        recheckUnsupported: 'This upstream exposes no billing endpoint, so probing is not needed. Click to re-check after changing the upstream.',
        stale: 'Stale',
        unsupported: 'Not applicable',
        unsupportedTitle: 'Probed: upstream exposes no billing endpoint',
        unsupportedHint: 'This upstream is not a Sub2API relay and has no rate endpoint to query, so this column does not apply to the account. It is re-checked automatically after the upstream changes.',
        failed: 'Failed',
        failedTitle: 'Probed: upstream declared rate was not retrieved',
        failureCount: '{count} consecutive failures',
        failureReasonUnknown: 'Reason: {code}',
        failureReason: {
          transport_unavailable: 'Reason: the local probe transport is unavailable',
          missing_api_key: 'Reason: the account has no API key configured',
          invalid_base_url: 'Reason: the upstream address is invalid',
          proxy_unavailable: 'Reason: the proxy bound to the account is unavailable',
          request_build_failed: 'Reason: the probe request could not be built',
          request_failed: 'Reason: the upstream request failed (network or timeout)',
          empty_response: 'Reason: the upstream returned an empty response',
          response_read_failed: 'Reason: the upstream response could not be read',
          response_too_large: 'Reason: the upstream response was too large',
          http_error: 'Reason: the upstream returned an error status',
          invalid_response: 'Reason: the upstream response is not recognizable rate data'
        },
        httpStatus: 'Upstream returned HTTP {value}',
        lastAttemptAt: 'Probed at: {value}',
        notProbed: 'Not probed',
        notProbedHint: 'This account has never been probed. Use the button on the right to probe now.',
        ...
        nextRecheckAt: 'Next automatic re-check: {value}',
      ...
      usageWindow: {
        ...
        mirasimQuota: {
          unprobed: 'Not probed',
          probe: 'Probe',
          probeTooltip: 'Query upstream /v1/limits once: an empty GET that spends no quota',
          probeNoSnapshot: 'Probe returned 200 without a snapshot',
          probeNotPerformed: 'No upstream probe ran; the stored reading was re-read',
          probeFailed: 'Last probe failed',
          probeFailedTooltip: 'The last /v1/limits probe failed, so there is still no reading here',
          staleReading: 'Reading may be stale',
          staleReadingTooltip: 'The last /v1/limits probe failed; the numbers below are the previous good reading, not current values',
          suspended: 'Upstream quota suspended',
          remainingUnknown: 'Remaining unknown',
          remaining: '{percent}% left',
          overUsed: '{percent}% used (over budget)',
          usedOfBudget: 'Used {used} / {budget}',
          noReadingTooltip: 'This probe did not return this window',
          resetAt: 'Resets {time}',
          resetUnknown: 'Reset time unknown',
          snapshotAt: 'Snapshot {time}',
          sourceLimits: 'From an active /v1/limits probe: absolute budgets included',
          sourceHeaders: 'From passive response-header sampling: utilization only, no absolute budget',
          planExpiresAt: 'Expires {date}',
          planAuthoritative: 'Authoritative tier read back from upstream',
          planClaimed: 'Tier claimed at import time, never verified upstream (known to over-state)',
          pendingUpgradeTooltip: 'Invitations met the threshold but the upgrade has not settled; still on the lower tier',
          restrictedAll: 'All models restricted',
          restrictedPartial: 'Some models restricted',
          scopeClaude: 'Claude (not Fable)',
          scopeFable: 'Fable',
          windowExhausted: '{window} exhausted',
          unmappedWindow: 'Unmapped window {name}',
          unmappedWindowTooltip: 'Upstream returned a window name outside 5h / 7d / 7d_claude / 7d_fable; it was not adopted into a known window'
        }
      },
      openaiQuotaReset: {
        count: 'Credits',
