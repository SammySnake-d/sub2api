...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		// mirasim subscription plan (plus/max + expiry). GET is the filterable
		// console view; the probe endpoints read the authoritative tier upstream.
		accounts.GET("/mirasim-plan", h.Admin.Account.ListMirasimPlans)
		accounts.GET("/mirasim-plan/settings", h.Admin.Account.GetMirasimPlanProbeSettings)
		accounts.PUT("/mirasim-plan/settings", h.Admin.Account.UpdateMirasimPlanProbeSettings)
		accounts.POST("/mirasim-plan/batch", h.Admin.Account.ProbeMirasimPlanBatch)
		accounts.GET("/:id", h.Admin.Account.GetByID)
		accounts.POST("", h.Admin.Account.Create)
		accounts.POST("/:id/duplicate", h.Admin.Account.Duplicate)
		accounts.POST("/check-mixed-channel", h.Admin.Account.CheckMixedChannel)
		accounts.POST("/import/codex-session", h.Admin.Account.ImportCodexSession)
		accounts.POST("/import/mirasim", h.Admin.Account.ImportMirasimAccounts)
